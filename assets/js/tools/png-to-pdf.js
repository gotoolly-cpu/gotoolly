document.addEventListener('DOMContentLoaded', function () {
    ...
    waitForLibs(function () {
        var fileInput = document.getElementById('png-input');
        ...
        function addFiles(files) {
            var valid = files.filter(function (f) { return f.type === 'image/png'; });
            if (valid.length === 0) {
                notify('Please select PNG files only', 'error');
                return;
            }
        ...
        async function startConversion() {
            if (images.length === 0) {
                notify('Please add PNG images first', 'error');
                return;
            }
            cancelled = false;
            ...
            try {
                ...
                for (var i = 0; i < images.length; i++) {
                    ...
                    var img = await pdfDoc.embedPng(images[i].data);
                    var imgW = img.width;
                    var imgH = img.height;
                    var maxW = pageW - 2 * m;
                    var maxH = pageH - 2 * m;
