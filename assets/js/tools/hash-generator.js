/* ============================================
   GO TOOLLY - HASH GENERATOR
   ============================================ */
document.addEventListener('DOMContentLoaded', function() {
    var input = document.getElementById('hg-input');
    var options = document.getElementById('hg-options');
    ...
    options.querySelectorAll('input[type="checkbox"]').forEach(function(cb) {
        cb.addEventListener('change', function() {
            ...
            if (liveMode.checked && input.value.trim()) generate();
        });
    });
    ...
    function generate() {
        ...
        if (algos.length === 0) { results.style.display = 'none'; return; }
        results.style.display = 'block';
        results.innerHTML = '<div style="text-align:center;padding:var(--space-4);color:var(--color-text-light)">Computing hashes...</div>';
        var promises = algos.map(function(algo) {
            return computeHash(text, algo).then(function(hash) {
        ...
        Promise.all(promises).then(function(hashes) {
            ...
            hashes.forEach(function(item) {
                ...
                var label = item.algo.toUpperCase();
                if (label === 'SHA1') label = 'SHA-1';
                else if (label === 'SHA256') label = 'SHA-256';
                else if (label === 'SHA512') label = 'SHA-512';
                row.innerHTML = '<span class="hg-label">' + label + '</span>' +
                    '<span class="hg-value">' + escapeHtml(item.hash) + '</span>' +
                    '<button class="hg-copy" data-hash="' + escapeHtml(item.hash) + '">Copy</button>';
                results.appendChild(row);
            });
            results.querySelectorAll('.hg-copy').forEach(function(btn) {
