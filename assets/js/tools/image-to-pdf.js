document.addEventListener('DOMContentLoaded', function () {
    var format = (document.getElementById('image-to-pdf-script') || {}).getAttribute ? (document.getElementById('image-to-pdf-script').getAttribute('data-format') || 'jpg') : 'jpg';
    var mimeMap = { jpg: 'image/jpeg', png: 'image/png' };
    var labelMap = { jpg: 'JPG', png: 'PNG' };
    var mime = mimeMap[format] || 'image/jpeg';
    var label = labelMap[format] || 'JPG';
    ...
    function waitForLibs(callback) {
        ...
        var interval = setInterval(function () {
            ...
            else if (attempts > 50) { clearInterval(interval); notify('Libraries failed to load. Please refresh the page.', 'error'); }
        }, 200);
    }
    ...
    waitForLibs(function () {
        ...
        function handleDrop(e) {
            ...
            var files = Array.from(e.dataTransfer.files).filter(function (f) { return f.type === mime; });
            addFiles(files);
        }

        function addFiles(files) {
            var valid = files.filter(function (f) { return f.type === mime; });
            if (valid.length === 0) { notify('Please select ' + label + ' files only', 'error'); return; }
            valid.forEach(function (file) {
                if (file.size > 50 * 1024 * 1024) { notify('File too large: ' + file.name, 'error'); return; }
        ...
        function updateUI() {
            ...
            images.forEach(function (img, idx) {
                ...
                div.innerHTML = '<img src="' + img.data + '" alt="' + img.name + '">' +
                    '<div class="thumb-info"><span class="thumb-name">' + img.name + '</span><span class="thumb-size">' + formatSize(img.file.size) + '</span></div>' +
                ...
                div.querySelector('.thumb-remove').addEventListener('click', function () { images.splice(idx, 1); updateUI(); });
                imageGrid.appendChild(div);
            });
        }

        async function startConversion() {
            if (images.length === 0) { notify('Please add ' + label + ' images first', 'error'); return; }
            cancelled = false;
            ...
            try {
                ...
                var embedFn = format === 'png' ? 'embedPng' : 'embedJpg';

                for (var i = 0; i < images.length; i++) {
                    if (cancelled) { notify('Cancelled', 'error'); break; }
                    ...
                    var img = await pdfDoc[embedFn](images[i].data);
                    var imgW = img.width;
                    var imgH = img.height;
                    var maxW = pageW - 2 * m;
                    var maxH = pageH - 2 * m;
